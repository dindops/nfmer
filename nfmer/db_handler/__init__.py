...
from nfmer.models import NFM_Event, Composition, EventCompositionLink, Composer, Event
...
class DatabaseHandler:
    ...
    def _clear_event_compositions(self, session: Session, event_id: str) -> None:
        statement = select(
            Composition).join(EventCompositionLink).where(EventCompositionLink.event_id == event_id)
        ...
        with Session(self.engine) as session:
            for event_id, event_data in parsed_results.items():
                ...
                if event_data.event_programme:
                    for composer_name, composition_name in event_data.event_programme.items():
    ...
    def get_compositions_by_event(self, event_id: str) -> list[Composition]:
        with Session(self.engine) as session:
            event = session.get(Event, event_id)
            return event.compositions if event else []
